...
public:
    ...
    void* FindAddressById(unsigned long long id) const {
        ...
        if (!FindOffsetById(id, offset)) return NULL;
    ...
    bool FindIdByOffset(unsigned long long offset, unsigned long long& result) const {
        ...
        if (itr == _rdata.end()) return false;
    ...
    bool Load() {
        ...
        if (!GetExecutableVersion(major, minor, revision, build)) return false;
    ...
    bool Load(int major, int minor, int revision, int build) {
        ...
        for (int i = 0; i < 4; i++) _ver[i] = read<int>(file);
        ...
        if (tnLen < 0 || tnLen >= 0x10000) return false;
        ...
        for (int i = 0; i < addrCount; i++) {
            ...
            if ((high & 8) != 0) q2 *= (unsigned long long)ptrSize;
...
bool DumpSpecificVersionAsCSV() {
    VersionDb db;

    int ver[] = {3, 16, 23, 39, 50, 53, 62, 73, 80, 97};
    int last_ver = ((sizeof(ver) / sizeof(ver[0])) - 1);

    for (int index = 0; index <= last_ver; index++) {
        if (!db.Load(1, 5, ver[index],
                     0)) {  // Try to load database of specified version regardless of running executable version.
            SKSE::log::error("Failed to load database for 1.5.{}.0!", ver[index]);
        } else if (db.Load(1, 5, ver[index], 0)) {
            // Write out a file called offsets-"version".txt where each line is the ID and offset.
            db.Dump(std::format("offsets-1.5.{}.0.csv", ver[index]));
            SKSE::log::info("Dumped offsets for 1.5.{}.0", ver[index]);
        }
    }
    return true;
}

bool DumpSpecificVersionAsTXT() {
    VersionDb db;
    ...
    for (int index = 0; index <= last_ver; index++) {
        if (!db.Load(1, 5, ver[index],
            ...
            // Write out a file called offsets-"version".txt where each line is the ID and offset.
            db.Dump(std::format("offsets-1.5.{}.0.txt", ver[index]));
            SKSE::log::info("Dumped offsets for 1.5.{}.0", ver[index]);
        }
    }
    return true;
