...
bool DumpSpecificVersionlibAsCSV() {
    VersionDblib db;
    ...
    for (int index =0; index <= last_ver; index++) {
        ...
        db.Dump(std::format("offsets-1.6.{}.0.csv",ver[index]));
        SKSE::log::info("Dumped offsets for 1.6.{}.0",ver[index]);
        }
    }
    return true;
}

bool DumpSpecificVersionlibAsTXT() {
    VersionDblib db;
    ...
    for (int index =0; index <= last_ver; index++) {
        ...
        // Write out a file called offsets-"version".txt where each line is the ID and offset.
        db.Dump(std::format("offsets-1.6.{}.0.txt",ver[index]));
        SKSE::log::info("Dumped offsets for 1.6.{}.0",ver[index]);
        }
    }
    return true;
